...
|-----|----------|---------|
| **`roar-kid-store.zip`** | Runtime files only, at the **top level** of the archive: `manifest.json`, `popup.html`, `options.html`, `background.js`, `content.js`, `dsp.js`, `options.js`, `popup.js`, `prompt.txt`, `prompt-builtin.txt`, `worklets/`, `icons/`. | **Upload this.** It is exactly what the extension needs and nothing else. |
...
cp roar-kid/manifest.json roar-kid/popup.html roar-kid/options.html \
   roar-kid/background.js roar-kid/content.js roar-kid/dsp.js \
   roar-kid/options.js roar-kid/popup.js \
   roar-kid/prompt.txt roar-kid/prompt-builtin.txt roar-kid-store/
mkdir -p roar-kid-store/worklets
cp roar-kid/worklets/*.js roar-kid-store/worklets/
