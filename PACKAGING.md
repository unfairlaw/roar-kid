...
|-----|----------|---------|
| **`roar-kid-store.zip`** | Runtime files only, at the **top level** of the archive: `manifest.json`, `popup.html`, `options.html`, `content.js`, `dsp.js`, `options.js`, `popup.js`, `prompt.txt`, `prompt-builtin.txt`, `worklets/`, `icons/`. | **Upload this.** It is exactly what the extension needs and nothing else. |
...
cp roar-kid/manifest.json roar-kid/popup.html roar-kid/options.html \
   roar-kid/content.js roar-kid/dsp.js roar-kid/options.js roar-kid/popup.js \
